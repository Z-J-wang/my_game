...
var Time: number | undefined;										//该全局变量用于存放setInterval(Auto_walk,$grade)，用于游戏停止
var $grade: number | undefined;										//该全局变量用于存在游戏等级的数字变量，主要用于设置行走的快慢
var $score = 0;                                 //建个全局变量用于存放分数
let foot_x!:number;
let foot_y!:number;
declare  var $: any;

const snake = $(document).ready(function(){
	Grade();									//初始化等级显示
	...
	$(window).keydown(function(event: { keyCode: any; }){			//为窗口添加键盘事件，
...
function create_foot(){
	...
	var $snakes = $(".background>div");
	do{
		foot_x = Math.floor(Math.random()*50)*20;			//随机生成一个坐标值
		foot_y = Math.floor(Math.random()*30)*20;
